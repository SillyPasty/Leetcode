...
struct Bucket{
    ...
    int maxv;
    int count;
    Bucket(int min_ = std::numeric_limits<int>::min(), int max_=std::numeric_limits<int>::max(), int count_=0){
        minv = min_; maxv = max_; count = count_;
    }
};
    ...
    int maximumGap(vector<int>& nums) {
        ...
        for(int x : nums){
            ...
            if(buckets[id].count == 0){
                ...
                if(x > buckets[id].maxv) buckets[id].maxv = x;
                if(x < buckets[id].minv) buckets[id].minv = x;
            }
            buckets[id].count++;
        }
        ...
        for(auto& b : buckets){
            ...
            if(b.minv - prev > max_gap) max_gap = b.minv - prev;
            prev = b.maxv;
        }
        return max_gap;
    }
